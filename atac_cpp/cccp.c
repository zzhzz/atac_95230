...
#ifndef ATAC_MODS		/* ATAC includes this stuff below */
#ifndef RLIMIT_STACK
# include <time.h>
#else
...
#ifdef ATAC_MODS /****************** A T A C ** M O D S *********************/
/*
* If ATAC_MODS is defined, all of the ATAC modifications are included.
* The ATAC_EXPAND, ATAC_LINENO, ATAC_PROTECTED, and ATAC_PPREDEFS are intended
* as documentation rather than to make the ATAC mods available a la cart.
*/

/* VMS only */
#ifdef VMS
#include <file.h>
#define ATAC_LIBINCLUDES
#define SYSTEM_C_LIBRARY	"SYS$LIBRARY:VAXCDEF.TLB"
#else /* non VMS */

/* MVS only */
#ifdef MVS
#define ATAC_LIBINCLUDES
#define SYSTEM_C_LIBRARY	"SYSLIB"
#else /* non MVS */
...
#define ATAC_EXPAND		/* annotated macro expansions */
#define ATAC_LINENO		/* line directives for readable files only. */
#define ATAC_PROTECTED		/* -% option prevents redef of selected macros*/
#ifndef MVS
#define ATAC_PPREDEFS		/* predefines from predef file */
#endif /* MVS */
#define ATAC_NOALLOCA		/* use xmalloc rather than alloca to get memory */

#ifndef ATAC_NOALLOCA
#ifdef USE_C_ALLOCA
#undef X3J11			/* don't let alloca.c see this. */
#include "alloca.c"
#endif /* USE_C_ALLOCA */
#endif /* ATAC_NOALLOCA */

#ifndef GETRLIMIT_SUPPORT
#undef RLIMIT_STACK
#endif /* GETRLIMIT_SUPPORT */

#ifndef TOOL_INCLUDE_DIR
#define TOOL_INCLUDE_DIR STANDARD_INCLUDE_DIR
#endif
#ifndef GCC_INCLUDE_DIR
#define GCC_INCLUDE_DIR STANDARD_INCLUDE_DIR
#endif
#ifndef GPLUSPLUS_INCLUDE_DIR
#define GPLUSPLUS_INCLUDE_DIR GCC_INCLUDE_DIR
#endif
...
#ifndef MVS
#undef S_ISREG	/* def is incompatible on UTS */
#endif /* MVS */

#ifdef ATAC_NOALLOCA
#undef  alloca
#define alloca xmalloc		/* ...just for simplicity */
#endif

#endif /* ATAC_MODS ***************** A T A C ** M O D S *********************/
...
#ifdef ATAC_LINENO /* { */
  char *trueFname;	/* Don't output line # directive for unreadable file */
  int trueLinenoOffset; /* This + lineno gives lineno in trueFname. */
  time_t trueMtime;	/* Last mod. time of trueFname file. */
#endif /* ATAC_LINENO } */
  char *fname;
...
#ifdef ATAC_EXPAND
static int atacExpand = 1;
#endif /* ATAC_EXPAND */

/* Symbols to predefine.  */

#ifdef ATAC_PPREDEFS
static char *predefFile = NULL;
#endif /* ATAC_PPREDEFS */
...
#ifdef ATAC_LINENO
static time_t getMtime PROTO((char *file));
#endif /* ATAC_LINENO */

#ifdef ATAC_EXPAND
static U_CHAR * embedOriginal PROTO(( U_CHAR *inStart, U_CHAR *inEnd, U_CHAR *outStart, U_CHAR *outMark, U_CHAR *outEnd));
#endif

#ifdef ATAC_PROTECTED
/*
* The -%name option defines a name to be protected.  In this case
* the name can only be #define'ed once and cannot be #undef'ed.  Attempts to
* redefine or undefine the name are silently ignored.  This may be useful
* in overriding macro expansions that are included in header files.
* For example, to override getc from stdio.h use -Dgetc=fgetc -%getc.
* The #define getc in stdio.h will be ignored.
*/
static char **protected = NULL;
#endif /* ATAC_PROTECTED */

#ifdef ATAC_LIBINCLUDES
static	char	*libInclude = NULL;	/* For #include from VMS lbr or MVS PDS */
static	int	libIncludeLen = 0;

static	char  libIncludePath[256] = SYSTEM_C_LIBRARY;
#endif /* ATAC_LIBINCLUDES */
     ...
     char **argv;
{
...
#ifdef ATAC_PROTECTED
  protected = (char **) xmalloc (argc * sizeof (char *));
  if (protected == NULL) memory_full();
  bzero ((char *)protected, argc * sizeof (char *));
#endif /* ATAC_PROTECTED */
  ...
  for (i = 1; i < argc; i++) {
      ...
      case '%':
	{
	  ...
	  if (argv[i][2] != 0)
	    protected[next++] = argv[i] + 2;
	  else if (i + 1 == argc)
	    fatal ("Assertion missing after -A option");
	  else
	    protected[next++] = argv[++i];
	}
	break;
#endif /* ATAC_PROTECTED */
	...
	{
#ifdef ATAC_LIBINCLUDES
	    static char uincludes[256];

	    strcat(uincludes, argv[i]+2);
	    strcat(uincludes, ",");

	    sprintf(libIncludePath, "%s%s",uincludes, SYSTEM_C_LIBRARY);
#else /* !ATAC_LIBINCLUDES */
	...
	fatal ("Invalid option `%s'", argv[i]);
      ...
      case 'a':
#ifdef ATAC_PPREDEFS
	if (strcmp(argv[i], "-atac_predefs") == 0) {
	    if (i + 1 == argc) {
		fatal ("Filename missing after -predefs option");
		break;
	    }
	    predefFile = argv[++i];
	    break;
...
#ifdef ATAC_EXPAND
	if (strcmp(argv[i], "-atac_noexpand") == 0) {
	    atacExpand = 0;
	    break;
	}
	else
  ...
  if (!inhibit_predefs) {
	...
	*termination = (char) save_char;
	...
	if (stat(predefFile, &sbuf) != 0)
	    pfatal_with_name(predefFile);
	if (sbuf.st_size != 0) {
	    f2 = fopen(predefFile, "r");
	    if (f2 == NULL)
		pfatal_with_name(predefFile);
	    buf = (char *)xmalloc(sbuf.st_size + 1);
	    while (fgets(buf, sbuf.st_size, f2) != NULL) {
		size_t len = strlen(buf);
		while ((len-- != 0) && isspace(buf[len]))
		    	buf[len] = '\0';
		if ((p = index(buf, ' ')) != 0)
		    *p = '=';
		make_definition(buf, &outbuf);
	    }
	    free(buf);
	    fclose(f2);
	}
  ...
  fp->nominal_fname = fp->fname = in_fname;
#ifdef ATAC_LINENO /* { */
  fp->trueFname = in_fname;
  fp->trueLinenoOffset = 0;
  fp->trueMtime = getMtime(in_fname);
#endif /* ATAC_LINENO } */
  fp->lineno = 1;
...
#ifdef ATAC_EXPAND /* { */
/*
* embedOriginal: Embed the text between inStart and inEnd at outStart
* enclosed in @< and @|.  Put @> at outMark.  If the length of the
* text is zero, do nothing.  Return new outEnd;
*/
static U_CHAR *
embedOriginal(inStart, inEnd, outStart, outMark, outEnd)
U_CHAR *inStart;
U_CHAR *inEnd;
U_CHAR *outStart;
U_CHAR *outMark;
U_CHAR *outEnd;
{
	...
	for (p = outEnd - 1; p >= outMark; --p)
		*(p + len + 6) = *p;
	*(outMark + len + 5) = '>';
	*(outMark + len + 4) = '@';
	for (p = outMark - 1; p >= outStart; --p)
		*(p + len + 4) = *p;
	*outStart = '@';
	*(outStart + 1) = '<';
	for (i = 0; i < len; ++i)
		outStart[i + 2] = inStart[i];
	*(outStart + len + 2) = '@';
	*(outStart + len + 3) = '|';
...
#ifdef ATAC_EXPAND /* { */
  /*
  * Save input and output positions at the beginning of an identifier
  * found in a source file (not in a macro expansion).	If it turns out
  * that this identifier begins a macro expansion, this is where we will
  * get/put a copy of the original text (enclosed as @< original @| expanded @>.
  * We are not interested in text that came from a macro expansion even
  * if it gets re-expanded.
  */
  U_CHAR *expandedInputStart = NULL;
  int expandedOutputStart = 0;	/* offset from op->buf */
#endif /* ATAC_EXPAND } */
...
#ifdef ATAC_EXPAND /* { */
      /*
      * If this is the begining of an identifier it might be the begining
      * of a macro expansion.  Save pointers needed for imbeding original.
      */
      if (ident_length == 0 && ip->fname) {
	expandedInputStart = ibp - 1;
	expandedOutputStart = obp - 1 - op->buf;
      }
#endif /* ATAC_EXPAND } */
      ident_length++;
...
#ifdef ATAC_EXPAND /* { */
	if (ip->fname) {
		check_expand(op, limit - expandedInputStart + 6);
		obp = op->bufp;
		obp = embedOriginal(expandedInputStart, ibp,
			op->buf + expandedOutputStart, obp, obp);
	}
#endif /* ATAC_EXPAND } */
	break;
      }
	...
	if (!pcp_outfile || pcp_inside_if) {
	  for (hp = hashtab[MAKE_POS (hash) % HASHSIZE]; hp != NULL;
...
#ifdef ATAC_EXPAND /* { */
		U_CHAR *expandedInputEnd = NULL;
		U_CHAR *expandedOutputEnd = NULL;
#endif /* ATAC_EXPAND } */
		while (1) {
		  ...
		  if (ibp == limit && ip->macro != 0) {
		    ...
		    if (ip->fname) {
			/*
			* End of a macro expansion.  If this expansion is
			* not further expanded, we should embed the original.
			* Save the necessary pointers until we know if there
			* is further expansion.
			*/
			expandedInputEnd = ibp;
			expandedOutputEnd = obp;
		    }
#endif /* ATAC_EXPAND } */
		  }
...
#ifdef ATAC_EXPAND /* { */
		  /*
		  * No macro here so embed the one we just finished.
		  */
		  if (expandedInputEnd) {
			op->bufp = obp;
			check_expand(op,limit - expandedInputStart + 6);
			obp = op->bufp;
			obp = embedOriginal(expandedInputStart,
				expandedInputEnd,
				op->buf + expandedOutputStart,
				expandedOutputEnd, obp);
		  }
#endif /* ATAC_EXPAND } */
		  /* Exit the for loop.  */
		...
		case '|':
	      ...
	      * Special symbols (e.g. __LINE__) are not rescanned so
	      * the end of the replacement text is detected immediately after
	      * the replacement.
	      */
	      if (hp->type != T_MACRO) {
		  ...
		  if (ip->fname) {
			check_expand(op, limit - expandedInputStart + 6);
			obp = op->bufp;
			op->bufp = embedOriginal(expandedInputStart, ibp,
				op->buf + expandedOutputStart, obp, obp);
		  }
	    } else
#endif /* ATAC_EXPAND ] */
...
#ifdef ATAC_NOALLOCA
  CHECK_DEPTH ({free (buf1); return obuf;});
#else /* ATAC_NOALLOCA */
...
#ifdef ATAC_LINENO /* { */
  /*
  * Need to treat # number like a #line directive so the readability of
  * the file name is checked.  To do this, fake the ident to be "line".
  */
  if (*bp >= '0' && *bp <= '9') {
	ident = (U_CHAR *)"line";
	after_ident = bp;
	ident_length = strlen((char *)ident);
  } else
#endif /* ATAC_LINENO } */
  {	/* for ATAC */
...
#ifdef ATAC_LIBINCLUDES
  char	*libInclude = NULL;	/* For #include from VMS lbr or MVS PDS */
  int	libIncludeLen = 0;
  char	*libIncludePath = SYSTEM_C_LIBRARY;
#endif /* ATAC_LIBINCLUDES */
  ...
  case '\"':
	...
	while (fin != limit) {
	 ...
	 file is coming from and put it on the front of the list. */
	  ...
	  int c = *limit;
	  *limit = '\0';
	  libInclude = (char *)libGet(fbeg, libIncludePath, &libIncludeLen,
				      &fname);
	  system_header_p = 1;
	  *limit = c;
      }
      if (libInclude) goto checkInclude;
      if (retried) {
	  error ("Cannot #include %.*s", limit - fbeg, fbeg, 0);
	  return;
      }
      goto fail;
#else /* not MVS */
...
#ifdef ATAC_LIBINCLUDES
    if (libInclude) {
	libFinclude(libInclude, libIncludeLen, fname, op);
	free(fname);	/* libInclude was freed by libFinclude(). */
    } else
#endif /* ATAC_LIBINCLUDES */
...
{
  char	*buf;
  FILE_BUF *fp = NULL;		/* For input stack frame */
...
#ifdef ATAC_LINENO /* { */
      /*
      * See if the file is older than the current input.
      * Kludge:  type is used to store the modification time.
      */
      hp->type = (enum node_type)getMtime(hp->value.cpval);
#endif /* ATAC_LINENO } */
    }
#ifdef ATAC_LINENO /* { */
    if ((int)hp->type > ip->trueMtime)
	older = 0;
#endif /* ATAC_LINENO } */
  } else if (*bp) {
    error ("invalid format `#line' directive");
...
#ifdef ATAC_LINENO /* { */
  /*
  * If the filename is not the name of file older than the current input file
  * fall back to the trueFname, i.e., the file we are actually reading from.
  * (This may not be the previous fname because the prevous fname may be a name
  * found in the preceeding #line directive for a file that IS older.)	In
  * either case, adjust lineno and trueLinenoOffset appropriately.
  * If the current input is stdin, everything is older.
  * If the file doesn't exist, the current input is older.
  */
  if (older == 0) {
	/* New fname not older -- fall back to trueFname. */
	ip->fname = ip->trueFname;
	ip->lineno += ip->trueLinenoOffset;
	ip->trueLinenoOffset = 0;
  } else {
	/* New fname older -- adjust line numbers. */
	ip->trueLinenoOffset += ip->lineno;
	ip->trueLinenoOffset -= new_lineno;
	ip->lineno = new_lineno;
  }
#else  /* } ATAC_LINENO { */
  ip->lineno = new_lineno;
#endif /* not ATAC_LINENO } */
     ...
     struct directive *keyword;
{
  ...
  while ((hp = lookup (buf, sym_length, -1)) != NULL) {
	...
	char **p;
	for (p = protected; *p; ++p)
	    if (strcmp((char *)(hp->name), *p) == 0) return 0;
    }
#endif /* ATAC_PROTECTED */
    delete_macro (hp);
  }
...
#ifndef ATAC_EXPAND /* { */
  /* [For ATAC this is moved up into rescan().] */
  /* it might not actually be a macro.	*/
  if (hp->type != T_MACRO) {
    special_symbol (hp, op);
    return;
...
initialize_char_syntax ()
{
  ...
  for (i=0; i<=255; i++)
	  ...
	  if (isalnum(i) || i == '_')
	      ...
	      {
		  is_idchar[i] = dollars_in_ident;
		  is_idstart[i] = dollars_in_ident;
	      }
      }
#endif
